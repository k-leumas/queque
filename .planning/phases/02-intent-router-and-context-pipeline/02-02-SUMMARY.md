...
plan: 02
subsystem: context-pipeline-and-candidate-selection
tags: [context, intent-gating, claude, ink, vitest, typescript]
requires:
  - phase: 02-intent-router-and-context-pipeline
    provides: typed request contracts and deterministic intent routing
provides:
...
affects: [phase-02, claude-provider, foreground-client, shell-selection]
tech-stack:
  added:
    - ink selection component built with useInput
    - @types/react for explicit selector typing
  patterns:
    - two-pass context pipeline
    - privacy-safe git porcelain parsing
    - candidate array parsing with fallback to single command
key-files:
  created:
