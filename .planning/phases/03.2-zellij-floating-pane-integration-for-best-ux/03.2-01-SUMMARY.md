...
plan: "01"
subsystem: testing
tags: [vitest, zsh, zle, zellij, fifo, tdd, esm-mocking]

requires:
  - phase: "01-shell-bridge-and-result-contract"
    provides: "result-writer.ts atomic rename, run-foreground.ts /dev/tty open, qq.zsh widget"
  - phase: "03.1-update-interface-and-interactivity-to-match-that-of-this-git"
    provides: "CandidateSelect TUI component, updated client flow"
provides:
...
affects:
  - "03.2-02-PLAN.md (Wave 2 implementation uses these tests as gate)"
  - "03.2-03-PLAN.md (Wave 3 uses zsh-widget tests as acceptance gate)"

tech-stack:
  added: []
  patterns:
    - "vi.hoisted() to capture real function refs before vi.mock factory runs — enables afterEach restoration"
    - "vi.fn() passthrough wrappers in mock factory for ESM non-configurable exports"
    - "Dynamic import inside tests to access vi.mock'd module for mocked property access"
    - "grep -c (count) instead of grep -qL for macOS-safe absence assertion"
