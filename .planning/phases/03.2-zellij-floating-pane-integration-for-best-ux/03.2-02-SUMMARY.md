...
plan: "02"
subsystem: client
tags: [typescript, fifo, zellij, ipc, result-writer, run-foreground, tdd, green]

requires:
  - phase: "03.2"
    plan: "01"
    provides: "TDD RED tests for FIFO-aware write and ZELLIJ env guard"
...
affects:
  - "03.2-03-PLAN.md (Wave 2 parallel — zsh widget rewrite, depends on result-writer FIFO write)"

tech-stack:
  added: []
  patterns:
    - "fsp.stat(path).then(s => s.isFIFO()) for named-pipe detection before write strategy selection"
    - "process.env['ZELLIJ'] !== undefined for Zellij runtime detection (ZELLIJ=0 in live sessions)"
    - "Optional chaining ttyHandle?.close() to handle null file handle safely"
    - "vi.clearAllMocks() in beforeEach to prevent cross-test mock call history leakage"
    - "ZELLIJ env isolation pattern in non-Zellij path tests: save/delete before, restore in finally"
