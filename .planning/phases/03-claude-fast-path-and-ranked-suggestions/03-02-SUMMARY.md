...
phase: 03
plan: 02
subsystem: client-contracts-registry
tags: [confidence-routing, error-handling, provider-registration, tdd]
dependency_graph:
  requires:
    - 03-01
  provides:
    - NormalizedRequest.confidence field for Phase 5 routing gate (INT-04)
    - Typed error ShellResult path (SAFE-01)
    - Claude provider backend registered in bootstrapBuiltins (PRV-02)
  affects:
...
tech_stack:
  added: []
  patterns:
    - Zod .extend() for schema field addition
    - fetchCandidates rejection writes typed error ShellResult via inner catch
    - registry clearProviderBackends() in test beforeEach for isolation
key_files:
  created: []
  modified:
    - src/contracts/request.ts
    ...
    - tests/registry-bootstrap.test.ts
decisions:
  - fetchCandidates rejection handled in inner catch (writes error FIFO + unmounts) rather than propagating to outer catch
  - ink render mock updated to only fire onSelect when candidates is non-null (fixes test race condition)
  - outer catch also writes error ShellResult for broader failure cases (classifyIntent/gatherContext failures)
metrics:
  duration: 499s
  completed: "2026-05-15T11:49:31Z"
  tasks: 2
  files: 5
---
