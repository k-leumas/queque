...
1. **`rerender()` vs wrapper-component setState for D-07**
   - What we know: Both work. `rerender()` is simpler and avoids ref leakage. UI-SPEC defers the choice to Claude's discretion.
   - What's unclear: Whether the vitest mock of `render` (currently not mocked) would interfere with `rerender()` in tests.
   - Recommendation: Use `rerender()` as the default. If tests require it, a thin wrapper component with `useState` is the fallback.

2. **Test update scope for `"shows immediate loading feedback"` test**
   - What we know: The test asserts `writeStreamWrites.join("").toContain("Que-Que is thinking...")`. This assertion will fail after D-05.
   - What's unclear: Whether to delete, replace, or update this test.
   - Recommendation: Replace the assertion with a check that `rerender` was called with a non-null `CandidateList`, or simply verify that the modal resolves without error. The planner should create an explicit task to update this test.
